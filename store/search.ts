...
export const mutations = {
  ...
  reset(_state: any) {
    ...
    if (_state.loadingState != LoadingState.ready)
...
export const actions = {
  async globalSearch(context: any, params: GlobalSearchParams) {
    ...
    context.commit("loadingState/setLoadingState", LoadingState.loading, {
      root: true,
    });
    ...
    setTimeout(async () => {
      if (context.state.requestIndex == _requestIndex) {
        ...
        context.commit("loadingState/setLoadingState", LoadingState.ready, {
          root: true,
        });
      }
    }, REQUEST_DELAY);
  ...
  async fetchProducts(context: any, params) {
    ...
    context.commit("loadingState/setLoadingState", loadingState, {
      root: true,
    });

    if (data?.ids && data.ids.length)
      context.commit("setProductResults", data.ids);
    else context.commit("setProductResults", []);
  },
};
