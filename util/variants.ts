...
export function getVariantTag(variant): string {
  const u = (a) =>
    `${a.measurementQuantity || 1} ${a.measurementUnit || "Stk"}`;

  if (variant.isSet) return "Set";
...
export function getVariantTitle(variant): string {
  const u = (a) =>
    `${a.measurementQuantity || 1} ${a.measurementUnit || "Stück"}`;

  if (variant.title) return variant.title;
  if (variant.isSet) return "Im Set";
  if (variant.numberOfSameArticle > 1)
...
export function isSpecialOffer(variant): boolean {
  if (!variant.recommendedRetailPrice) return false;

  return (
    variant.price / variant.recommendedRetailPrice < 1 - GLOBAL.specialOfferMin
  );
}
