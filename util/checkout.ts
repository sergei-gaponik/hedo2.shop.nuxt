export function getShippingMethodPrice(shippingMethod, subTotal) {
  if (
    ...
    shippingMethod.freeShippingMin < subTotal
  )
    return 0;
...
export function getDefaultShippingMethod(shippingMethods, subTotal) {
  ...
  const cheapest = shippingMethods.filter(
    (a) => getShippingMethodPrice(a, subTotal) == lowestPrice
  );

  if (cheapest.length == 1) return cheapest[0];
  else return cheapest.sort((a, b) => a.deliveryTimeTo - b.deliveryTimeTo)[0];
}
