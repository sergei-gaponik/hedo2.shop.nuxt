...
export default {
  computed: {
    ...
    priceRange() {
      return (
        this.$store.state.filters.priceRange || [0, GLOBAL.maxPriceRangeValue]
      );
    },
    maxPriceRange() {
      return (
        this.$store.state.filters.maxPriceRange || [
          0,
          GLOBAL.maxPriceRangeValue,
        ]
      );
    },
    lineLeft() {
      const perc =
        ((this.minPrice - this.leftBoundry) /
          (this.rightBoundry - this.leftBoundry)) *
        100;
    ...
    lineRight() {
      const perc =
        ((this.rightBoundry - this.maxPrice) /
          (this.rightBoundry - this.leftBoundry)) *
        100;
    ...
    caption() {
      if (this.minPrice == 0) {
        ...
        if (this.maxPrice == GLOBAL.maxPriceRangeValue)
          ...
          return `${this.$t("from")} ${this.minPrice} ${
            this.currencySymbol
          } ${this.$t("to")} ${this.maxPrice} ${this.currencySymbol}`;
      }
    },
  },
  data() {
...
.a-slider:active::-webkit-slider-thumb,
.a-slider:active::-moz-range-thumb {
  width: 18px;
  height: 18px;
}
</style>
