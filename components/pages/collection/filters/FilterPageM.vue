<template>
  <div class="a-page" v-if="selectedCategory != null">
    <div
      class="a-container"
      :style="{ transform: showFilters ? 'translateX(-100%)' : 'none' }"
    >
      ...
      <div :class="['a-filters', $device.isMobile ? '' : 'a-filters-t']">
        <div class="a-filters-header" @click="closeFilters()">
    ...
    <button
      :class="[
        'a-button',
        filtersChanged ? 'a-button-apply' : 'a-button-close',
        $device.isMobile ? '' : 'a-button-t',
      ]"
      @click="apply"
    >
...
export default {
  ...
  methods: {
    apply() {
      if (!this.filtersChanged) this.$store.commit("nav/closeAllDrawers");
      this.$store.commit("search/reset");

      const categories = this.$store.state.filters.availableCategories;
      const query = selectedFiltersToQueryParams(
        categories,
    ...
    setFilterSelected(id, selected) {
      if (selected) this.selectedFilters.push(id);
      else this.selectedFilters = this.selectedFilters.filter((a) => a != id);
    },
    closeFilters() {
  ...
  computed: {
    selectedCategory() {
      if (
        this.$store.state.filters.availableCategories.length >=
