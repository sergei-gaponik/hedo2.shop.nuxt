...
<div :class="['a-container', $device.isMobile ? 'a-container-m' : '']">
  <div v-if="!$device.isMobile" class="a-results">
    ...
    <div class="a-options">
      <div 
        ...
        :class="['a-caption', 'a-option', option == limit ? 'a-selected' : '']"
      >
        {{ option }}
      </div>
    </div>
  ...
  <div class="a-pagination">
    <div class="a-iconbutton" @click="clickHandler(() => $emit('prevPage'))">
      <chevron-left-icon height=40 :color="hasPrevPage ? 'var(--c-gray-2)' : 'var(--c-gray-3)'" />
    </div>
    <div class="a-caption">
      {{ $t("page") }} <span class="a-num">{{ page }}</span> {{ $t("pageOf") }} <span class="a-num">{{ totalPages }}</span>
    </div>
    <div class="a-iconbutton" @click="clickHandler(() => $emit('nextPage'))">
      <chevron-right-icon height=40 :color="hasNextPage ? 'var(--c-gray-2)' : 'var(--c-gray-3)'" />
    </div>
  </div>
</div>
</template>
