<template>
  <div class="a-orderinfo panel pointer" @click="$emit('click')">
    <div class="flex-h-c">
      <cancelled-icon v-if="statusCancelled" height=20 color="var(--c-gray-2)" />
...
export default {
  ...
  computed: {
    ...
    itemCountCaption(){
      ...
      if(items == 1)
        return `1 ${this.$t('item')}`
      else
        return `${items} ${this.$t('items')}`
    },
    total(){
      return toMoney(Math.round((this.order.shippingCost + this.order.subTotal) * 100) / 100, this)
    },
    orderDate(){
      const options = { month: 'numeric', day: 'numeric', year: 'numeric' }


      return new Date(this.order._created).toLocaleDateString(this.$i18n.localeProperties.numberFormat, options)
    }
  }
}
</script>
