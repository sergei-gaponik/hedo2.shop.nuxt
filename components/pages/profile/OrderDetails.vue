    ...
    <div class="container">
      ...
      <div class="a-items mb2">
        ...
        <show-more
          ...
          v-model="showAll"
          :showMoreCaption="
            $t('showAllNItems').replace('{n}', order.lineItems.length)
          "
        />
      </div>
      <div :class="$device.isMobile ? 'flex-v mb2' : 'grid-2 mb2'">
        <div>
          ...
          <address-info :address="order.shippingAddress" hideIcon />
        </div>
        <div>
          <h6>{{ $t("billingAddress") }}</h6>
          <address-info :address="billingAddress" hideIcon />
        </div>
      </div>
      <cart-total
        class="mb2"
      ...
      <primary-button @click="trackDelivery()">{{
        $t("trackDelivery")
      }}</primary-button>
    </div>
  </div>
...
export default {
  ...
  computed: {
    visibleLineItems() {
      if (this.showAll) return this.order.lineItems;
      else return this.order.lineItems.slice(0, 3);
    },
    billingAddress() {
      if (this.order.billingAddressMatchesShippingAddress)
        return this.order.shippingAddress;
      else return this.order.billingAddress;
    },
  },
  methods: {
    trackDelivery() {},
