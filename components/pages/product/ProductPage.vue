  ...
  <div>
    <div v-if="$device.isMobile">
      ...
      <drawer-bottom-m :show="$store.state.nav.cartDrawerOpen">
        <add-to-cart-page :product="product" :variant="selectedVariant" />
      </drawer-bottom-m>
    </div>
    <div :class="containerClass">
      <div v-if="!$device.isMobile" class="td-split-sticky">
        <portal to="body">
          <pop-up
            :show="$store.state.nav.cartDrawerOpen"
            width="480px"
            height="600px"
          >
            <add-to-cart-page :product="product" :variant="selectedVariant" />
      ...
      <div :class="$device.isMobile ? '' : 'td-split-sticky'">
        ...
        <p class="text">
          {{
            `${$t("deliveryTime")}: ${$i18n.localeProperties.deliveryTime.join(
              " - "
            )} ${$t("days")}`
          }}<br />
        </p>
        ...
        <lazy-wrapper
          :loadingState="similarProductsLoadingState"
          v-if="$device.isMobile"
        >
          <div v-if="similarProducts && similarProducts.length">
            <h2 class="h2">{{ $t("similarProducts") }}</h2>
...
export default {
  ...
  methods: {
    async addToCart(quantity) {
      ...
      const r = await this.$store.dispatch("cart/addToCart", {
        variant,
        product,
        quantity,
        price,
  ...
  computed: {
    ...
    maxQuantity() {
      return Math.min(
        this.selectedVariant.availableQuantity,
        this.selectedVariant.maxQuantity || 99
      );
    },
    eanCaption() {
