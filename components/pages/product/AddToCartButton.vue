...
export default {
  ...
  computed: {
    maxQuantityToAdd() {
      const lineItem = this.$store.state.cart.lineItems.find(
        (a) => a.variant == this.selectedVariant._id
      );

      return lineItem ? this.maxQuantity - lineItem.quantity : this.maxQuantity;
    },
    disabled() {
      if (this._created) return this.maxQuantityToAdd == 0;
      else return true;
    },
  },
  created() {
  ...
  methods: {
    _action() {
      if (this.disabled) this.$refs["addtocart" + this._uid].shake();
      else this.$refs["addtocart" + this._uid].press();

      this.$emit("action", this.quantity);
      this.quantity = 1;
    },
    incQuantity() {
      if (this.quantity < this.maxQuantityToAdd) this.quantity++;
    },
    decQuantity() {
