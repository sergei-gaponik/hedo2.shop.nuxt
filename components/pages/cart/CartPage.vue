      ...
      <div v-if="cartLoadingState == 'ready'">
        ...
        <div v-if="!cartEmpty" :class="containerClass">
          <div
            :class="[
              'a-cartitems',
              !$device.isMobile ? 'td-split-sticky' : 'mb4',
            ]"
          >
            <cart-item
...
export default {
  ...
  computed: {
    containerClass() {
      ...
      else if (this.$device.isTablet) return "td-split";
      else return "td-split-3-2";
    },
    cartItems() {
  ...
  methods: {
    async refreshLineItems() {
      ...
      try {
        ...
        if (r.loadingState != LoadingState.ready) throw new Error();

        for (const { variant, availableQuantity } of r.data.variants) {
          const _variant = this.variants.find((a) => a._id == variant);

          _variant.availableQuantity =
            _variant.availableQuantity > availableQuantity
              ? availableQuantity
              : _variant.availableQuantity;
        }
        //#endregion
        ...
        const maxQuantity = (cartItem) =>
          cartItem.variant.maxQuantity != null
            ? Math.min(
                cartItem.variant.maxQuantity,
                cartItem.variant.availableQuantity
              )
            : cartItem.variant.availableQuantity;

        const notEnoughQuantityCartItems = cartItems.filter(
          (a) => maxQuantity(a) < a.quantity
        );
        ...
        if (notEnoughQuantityCartItems.length) {
          ...
          this.$store.dispatch(
            "notifications/error",
            this.$t("cartQuantityChangedNotification")
          );
          result = false;
        }
        //#endregion
        ...
        const r2 = await instanceHandler({
          path: "signLineItems",
          args: {
            lineItems: cartItems.map((a) => ({ variant: a.variant._id })),
          },
        });

        if (r2.loadingState == LoadingState.error) throw new Error();
        ...
        if (updateFlag) {
          this.$store.dispatch(
            "notifications/error",
            this.$t("priceChangeNotification")
          );
          return false;
        }
      } catch (e) {
