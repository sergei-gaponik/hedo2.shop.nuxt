<template>
  <div v-if="banners.length">
    <div class="a-banner">
      ...
      >
        <chevron-left-icon height="36" color="white" />
      </div>
      ...
      >
        <chevron-right-icon class="a-icon" height="36" color="white" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    banner() {
      if (!this.banners.length) return null;
      else return this.banners[this.position];
    },
  },
  data() {
    return {
  ...
  methods: {
    clickHandler,
    prevItem() {
      if (this.position <= 0) this.position = this.banners.length - 1;
      else this.position--;
    },
    nextItem() {
      if (this.position >= this.banners.length - 1) this.position = 0;
      else this.position++;
    },
  },
  async fetch() {
    ...
    this.banners =
      r.data?.banners?.sort((a, b) => b.priority - a.priority) || [];
