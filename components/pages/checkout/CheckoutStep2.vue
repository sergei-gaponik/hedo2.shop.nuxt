<template>
  <lazy-wrapper>
    <div>
      ...
      />
      <div
        :class="$device.isMobile ? 'container-m' : 'container-slim-d td-split'"
      >
        <div :class="$device.isMobile ? '' : 'td-split-sticky'">
          ...
          <div>
            ...
            <check-box
              ...
              @input="
                (v) =>
                  setShippingInfo('billingAddressMatchesShippingAddress', v)
              "
            >
...
export default {
  ...
  methods: {
    submit() {
      ...
      if (this.isValid) this.$emit("nextStep");
    },
  ...
  async fetch() {
    ...
    const shippingAddress =
      this.addresses.find((a) => a.defaultShippingAddress) || this.addresses[0];

    if (!this.shippingInfo.shippingAddress && shippingAddress)
      this.setShippingInfo("shippingAddress", shippingAddress);

    const billingAddress =
      this.addresses.find((a) => a.defaultBillingAddress) || this.addresses[0];
