  ...
  <div :class="$device.isMobile ? 'container-m' : 'container td-split'">
    ...
    <div :class="$device.isMobile ? '' : 'td-split-sticky'">
      ...
      <div :class="$device.isMobile ? 'main-button-m' : 'mt4'">
        <main-button :disabled="!isValid" @click="submit()">{{ $t('goToPaymentProvider') }}</main-button>
      </div>
      <div v-if="!$device.isMobile" class="mb4 mt">
        <divider-or />
        <div id="paypal-checkout"></div>
      </div>
...
export default {
  ...
  components: { MainButton, LazyWrapper, PaymentMethodSelector, OrderSummery1, OrderSummery2, DividerOr },
  computed: {
    paymentInfo(){
  ...
  async mounted(){
    ...
    const paypal = await loadScript({
      ...
      'currency': 'GBP',
      'disable-funding': 'credit,card,bancontact,blik,eps,giropay,ideal,mercadopago,mybank,p24,sepa,sofort,venmo',
      'locale': 'de_DE'
    })
    ...
    this.$nextTick(function() {
      ...
      paypal.Buttons({
        ...
        onApprove: () => {
          this.$router.push(this.localePath(`/checkout/success?sid=${uuid}&p=paypal`))
        }
      })
