  ...
  <pop-up :show="$store.state.confirmDialog.visible" height="min-content" width="90%" maxWidth="420px" zIndex="10000">
    <div class="container">
      ...
      <div class="btn-group-h">
        ...
        <secondary-button @click="_resolve(false)">{{ $t('cancel') }}</secondary-button>
      </div>
    </div>  
  </pop-up>
</template>
...
export default {
  components: { PopUp, PrimaryButton, SecondaryButton },
  methods: {
    _resolve(state){
      this.$store.commit("confirmDialog/setStatus", state)
    }
  }
}
</script>
