<template>
  <client-only>
    ...
    >
      <div class="mb2 mt" v-if="loadingState == 'loading' || products.length">
      ...
      <lazy-wrapper>
        <div :class="$device.isMobile ? 'container-m' : 'container'">
          ...
          <div
            :class="[
              'a-results',
              !$device.isMobile && articles.length ? 'a-results-td' : '',
            ]"
          >
            <div
              :class="[
            ...
            <div v-if="articles.length">
              ...
              <article-list :articleSearchResults="articles" />
            </div>
          </div>
        </div>
      </lazy-wrapper>
  ...
  created() {
    if (process.client) {
      window.addEventListener("keydown", async (e) => {
        if (
          this.$store.state.loadingState.loadingState == LoadingState.loading ||
        ...
        if (e.key == "Enter" && query) {
          ...
          if (maxScores.length && maxScores[0][0] != "products") {
            ...
            if (map.hasOwnProperty(key)) href = this.href(key, map[key][0]);
