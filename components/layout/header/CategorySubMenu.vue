  ...
  <div class="a-inner">
    <div class="a-menu">
      ...
      <div v-else-if="category" class="a-left mb2">
        <div v-for="child in category.children" :key="child._id" class="a-col">
          <span class="pointer" @click="href('/c/' + child.handle)">
            <h4>{{ child.title || child.name }}</h4>
          </span>
          <div class="a-subitems">
            <span @click="href('/c/' + child.handle)">{{ $t('allProducts') }}</span>
            <span
            ...
            <span @click="href('/c/' + child.handle + '?sale=1')">{{ $t('saleCollection') }}</span>
          </div>
        </div>
      </div>
    </div>
